...
This document is **Methodology v1.5**. The `METHODOLOGY_VERSION` constant that stamps
scorecards stays at `"v1.4"`: v1.5 changes item-validation publication rules, not the scoring
of any dimension, so no scorecard's composite depends on it — the same reason v1.3 (Tier 2)
never bumped the constant.
...
| v1.5 | 2026-07-19 | **Item-validation IRR rules stated.** **(a) Floor raised 0.60 → 0.70.** The v1.3 row above records the Tier 1 gate as IRR ≥ 0.60; three other documents said 0.70 and the implementation now enforces 0.70 (`validation/irr.py`). 0.70 governs — it is also the conventional "substantial agreement" bar for Cohen's kappa, and the lower figure was never deliberately chosen. The v1.3 row is left as written; it is the historical record of what that version said. **(b) `irr_score` is a pair-and-batch statistic, not an item statistic.** Cohen's kappa is undefined for a single item. It is computed once per rater pair, quadratic-weighted over the 1–5 cultural rubric across every item that pair both rated, and that one value is written onto each item in the batch. A pair sharing fewer than **10** items has no defensible reliability number: `irr_score` stays null and the item is not Tier 1 — never estimated, extrapolated, or borrowed from another pair. Kappa is computed on `cultural_score` only. **(c) A factual-accuracy disagreement forces adjudication regardless of kappa.** Two validators splitting on whether an item is factually correct is a blocking defect, not rater noise; it is never averaged into the reliability statistic and it overrides a passing kappa, up to and including 1.00. Founder-approved; historical scorecards unaffected — no scored item's composite depends on `irr_score`. |
